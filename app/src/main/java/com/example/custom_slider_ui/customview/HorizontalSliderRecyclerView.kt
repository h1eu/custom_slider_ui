package com.example.custom_slider_ui.customview
...
class HorizontalSliderRecyclerView @JvmOverloads constructor(
    context: Context,
    ...
    private fun onScrollChanged() {
        post {
            (0 until childCount).forEach { pos ->
                val child = getChildAt(pos)
                val childrenCenterX = (child.left + child.right) / 2
                val scaleValue = getGaussianScale(childrenCenterX,1f,1f,150.0)
    ...
    private fun getGaussianScale(
        ...
        return (Math.pow(
            Math.E,
            -Math.pow(childrenCenterX - recyclerCenterX.toDouble(), 2.toDouble()) /
